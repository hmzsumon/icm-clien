...
const WithdrawSecurity = ({ openModal, setOpenModal, handleSubmit }: any) => {
	...
	const handleConfirm = () => {
		if (codeError || passCodeError || !code || !passCode) {
			return;
		}
		handleSubmit();
		setOpenModal(false);
					...
					<Modal.Body>
						<div className='space-y-6'>
							...
							<div>
								...
								/>
								{verifySuccess ? (
									<p className='text-right text-xs flex justify-end items-center font-semibold mr-2 mt-1 cursor-pointer text-icm-green'>
									...
									<small
										...
										className='text-right flex justify-end font-semibold mr-2 mt-1 cursor-pointer text-icm-green'
									>
										{resendDisabled
							...
							<div>
								...
								<TextInput
									...
									helperText={passCodeError && passCodeErrorText}
									readOnly={passCodeVerified}
								/>

								{passCodeVerified ? (
									<p className='text-right text-xs flex justify-end items-center font-semibold mr-2 mt-1 cursor-pointer text-icm-green'>
										<MdOutlineDone className=' text-xl mr-1 font-bold' />
										Pass code verified successfully
									</p>
								) : (
									<Link
							...
							<div className='w-full'>
								<Button
									className='w-full bg-icm-green text-gray-800 font-bold'
									disabled={codeError || passCodeError || !code || !passCode}
